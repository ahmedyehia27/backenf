...
3. التنوين (قاعدة ذهبية إلزامية): أي كلمة منونة سمعتها في الصوت يجب كتابة "تنوين الألف" (تنوين الفتح ـاً) عليها بشكل دقيق وإلزامي مثل: (شكراً، جداً، حقاً، فعلاً، دائماً، مثلاً، أهلاً، مرحباً، طبعاً، غالباً، تقريباً، تماماً، فوراً، خصوصاً، عموماً، مسبقاً، أحياناً، أبداً، مجدداً، أولاً، ثانياً، ثالثاً، حالياً، شخصياً، نهائياً، يومياً، سنوياً، شهرياً، سابقاً، لاحقاً، قانونياً، رسمياً، حرفياً، عاماً، يوماً، شهراً، وقتاً، شيئاً، جزءاً). ممنوع كتابة حرف النون في نهاية التنوين! وممنوع نسيان التنوين (لا تكتب 'جدا' أو 'شكرا' بل 'جداً' و 'شكراً').
...
5. الكلمات الإنجليزية: اكتب الكلمات والمصطلحات والأسماء الإنجليزية بحروفها الإنجليزية الصحيحة تماماً وبترتيبها السليم (مثل: iPhone, AI, ChatGPT, YouTube, Google, CEO, B2B, ROI, Python, API).
6. الهمزات: صحح همزات الوصل والقطع إذا تطلب الأمر. (هذي تبقى هذه، فيه تبقى في، كذا تبقى كدا).
7. التنظيف: احذف أي تأتأة أو كلمات غير مكتملة أو تكرار مثل (اااا، الـ، ال ال، في ال في ال) وأي شرطات (--) في نهايات السطور.
8. الأرقام: اكتب التواريخ والأرقام حسابياً وليس بالحروف (مثال: 2024 وليس ألفين وأربعة وعشرين).
9. الإخراج: أخرج النص المصحح فقط داخل كود بلوك ```، بحيث يكون كل سطر مستقلاً وبنفس ترتيب المدخلات.
...
{formatted_scribe_text}"""
    ...
    def fix_arabic_tanween(text: str) -> str:
        if not text:
            return ""
        tanween_map = {
            r"\bشكرا\b": "شكراً",
    ...
    for i in range(len(corrected_chunks)):
        ...
        clean_new_line = fix_arabic_tanween(clean_new_line)
...
def buffer_channels_proxy(req: BufferChannelsProxyReq):
    ...
    try:
        ...
        if not orgs:
            return {"channels": [], "status": "success", "message": "No organizations found"}
        ...
        for org in orgs:
            ...
            """
            cres = requests.post("https://api.buffer.com", headers=headers, json={"query": chan_query}, timeout=20)
            cdata = cres.json()
            for ch in cdata.get("data", {}).get("channels", []):
                channels.append({
                    "id": ch.get("id"),
                    ...
                    "isQueuePaused": ch.get("isQueuePaused", False),
